<template>
    <div class="p-fluid p-grid mt-4">
        <form @submit.prevent="editPhase()" v-if="phase">
            <!-- numero de la fase -->
            <div class="form-group py-2 mb-4">
                <div class="p-field p-col-12">
                    <span
                        class="h3 border-1 border-end border-bottom pe-3 pb-3"
                    >
                        {{ labels.phase }}
                        <span class="text-primary"
                            >{{ labels.numberPhase }} {{ phase.number }}</span
                        >
                    </span>
                </div>
            </div>
                            ...
                            >
                                ...
                                <td>
                                    <button
                                        ...
                                        v-if="video.via === TYPE_UPLOAD.url"
                                    >
...
export default {
    ...
    props: {
        phase: {
            type: Object,
            required: true,
            description: "Objeto con las propiedades de la fase",
        },
    },
    ...
    data() {
        return {
            ...
            // definir si se esta editando un video o no
            edit: false,
        };
    },
    ...
    mounted() {
        ...
        // cargar todos los datos para ser editados
        this.loadPhaseData();
    },

    methods: {
        /**
         * Cargar los datos de la fase en el formulario
         */
        loadPhaseData() {
            // cargar los datos de la fase en el formulario
            this.form.title = this.phase.title;

            // cargar los videos de la fase en la tabla
            this.phase.videos.forEach((video) => {
                if (video.url) {
                    this.videos.push({
                        id: video.id,
                        name: video.url,
                    ...
                    this.videos.push({
                        ...
                        type: video.type,
                        size: video.size,
                        sizeMB: this.getSizeMb(video.size),
         ...
         */
        editVideo(index) {
            // tomar el video seleccionado
            const video = this.videos[index];

            if (!video) {
                this.$toast.error(this.appMessages.noVideoSelected);
            }

            // si es por url
            if (video.via === this.TYPE_UPLOAD.url) {
                this.viaUrl();
                this.clearNullValuesToUrl();
                this.videosUrl.push({ url: video.url });
            }

            // indicar que se esta editando un video
            this.edit = true;

            // eliminar el video de la tabla
        ...
        checkVideoFile(event) {
            ...
            this.videosFile.forEach((file) => {
                ...
                if (continuar) {
                    ...
                    file.sizeMB = this.getSizeMb(file.size);
                }
            });
        ...
        loadVideosToTable() {
            ...
            // Ninguna carga seleccionada
            this.isFiles = false;
            this.isUrl = false;

            // indicar que no se esta editando
            this.edit = false;
        },
        ...
        getFormData() {
            ...
            this.videos.forEach((video, index) => {
                ...
                // incluir solo los videos modificados,
                // los que no poseen id
                if (!video.id) {
                ...
                // incluir los videos a ignorar, para no eliminarlos
                formData.append(`videos[ids][${index}]`, video.id);
            });
        ...
        /**
         * Obtener los datos formateados del tamaño
         * del archivo en mb
         */
        getSizeMb(size) {
        ...
        /**
         * Limpiar los valores vacios del array de videos
         * por url
         */
        clearNullValuesToUrl() {
            this.videosUrl = this.videosUrl.filter((video) => video.url.length);
        },

        /**
         * Guardar los datos y editar fase
         */
        editPhase() {
            ...
            axios
                .post(route("phases.updatePhase", this.phase.id), formData, {
                    headers: {
